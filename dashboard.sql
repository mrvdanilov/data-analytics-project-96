...
WITH clicks AS (
    SELECT COUNT(DISTINCT visitor_id) AS cnt_clk
    FROM sessions
),

lds AS (
    SELECT COUNT(DISTINCT visitor_id) AS cnt_ld
    FROM leads
),

purchases AS (
    SELECT COUNT(DISTINCT visitor_id) AS cnt_prs
    FROM leads
    WHERE status_id = 142 OR closing_reason = 'Успешно реализовано'
)

SELECT
    CAST(cnt_ld AS FLOAT) / cnt_clk AS click_to_lead,
    CAST(cnt_prs AS FLOAT) / cnt_ld AS lead_to_purchase
FROM clicks
cross join lds
...
SELECT
    PERCENTILE_CONT(0.9) WITHIN GROUP (ORDER BY l.created_at - s.visit_date) AS p90_days
FROM sessions AS s
